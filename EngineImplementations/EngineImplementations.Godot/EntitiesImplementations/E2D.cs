...
public class E2D : IEntities.I2D
{
    public EntityComponent<INode2D> Node()
    {
        ...
        return new EntityComponent<INode2D>(new GdEntity(node2d), new CNode2D(node2d));
    }

    public EntityComponent<ISprite2D> Sprite(ITexture? texture = null)
    {
        var sprite2d = new Sprite();
        return new EntityComponent<ISprite2D>(new GdEntity(sprite2d), new CSprite2D(sprite2d, texture));
    }
    ...
    public EntityComponent<ITrigger2D> Trigger(Collider2D? collider2D = null)
    {
        ...
        return new EntityComponent<ITrigger2D>(new GdEntity(trigger2D), cTrigger2d);
    }
}
